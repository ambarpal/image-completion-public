...
class DataLoader:
    def __init__(self, dataset_name='MNIST'):
        ...
        if not os.path.isdir(data_processed_path):
            ...
            if(dataset_name != 'CELEBA'):
                ...
                np.save(open(data_processed_path + '/data.npz', 'w'), self.data)
                np.save(open(data_processed_path + '/labels.npz', 'w'), self.labels)
        else:
            self.data = np.load(data_processed_path + '/data.npz')
            if dataset_name != 'CELEBA':
                self.labels = np.load(data_processed_path + '/labels.npz')
    ...
    def load_corrupted_batch_X(self, batch_size, mask_2D, update_iterator = True):
        batch_x = np.zeros((batch_size, 64, 64,3))
        length = self.data.shape[0]
        if self.curIdx + batch_size < length:
            batch_x = self.data[self.curIdx:self.curIdx + batch_size]
        ...
        batch_x = np.multiply(batch_x, mask)
